...
SERVER_ADDRESS = os.environ.get('SERVER_ADDRESS', 'localhost:8082')
...
class FLClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        global_model = set_params(self.model, parameters)
        global_model.fit(X_loc, y_loc)
        y_pred = global_model.predict(X_loc)
        loss_val = 1 - float(__import__('sklearn.metrics', fromlist=['accuracy_score'])
                            .accuracy_score(y_loc, y_pred))
        return get_params(global_model), len(y_loc), {'loss': loss_val}
